...
for patient in tqdm(list_of_patients):
    ...
    img = np.load(path+patient+"/"+"img.npy")[:4]
    img = img / img.max()
    inversion_time = np.load(path+patient+"/"+native_or_enhanced+"inversion_time_1.npy")[:4] / 1000.0
    ...
    vendor = np.matlib.repmat(2, 160 * 160, 1)
    ...
    molli = loadmat(path + patient + "/t1_enhanced_mid_1")
